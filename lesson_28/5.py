...
print("\n Ismi K dan boshlanadigan hodimlarni oyligini 2-karra oshiri")
for person in data:
    if person["full_name"][0] == "K":
        salary = float(person["salary"].replace("$", ""))
        person["salary"] = f"${salary * 2:.2f}"

for person in data:
    print(f"{person['full_name']} - {person['company']} - {person['position']} - {person['salary']}")
